...
const MiniCard: React.FC<MiniCardProps> = ({
  ...
  return (
    <div
      onClick={() => setStateProduct(product)}
      className={`p-4 min-w-[7.2rem] max-w-[7.2rem] m-5 ${
        stateProduct.id === product.id
          ? "ring-2 ring-Lightblue-600  bg-Lightblue-300"
          : " bg-Lightblue-100"
      } gap-3 z-5 select-none flex flex-col min-h-68 transition-all duration-300 hover:scale-105 hover:ring-2 ring-Lightblue-300`}
    >
      <p className="font-titilium text-sm  text-Lightblue-950 font-semibold">
        id: {product.id}
      </p>
      <Image
        width={380}
        ...
        alt="no image"
      />
      <p className="font-titilium text-md text-Lightblue-900 font-semibold">
        {product.name}
      </p>
    </div>
  );
