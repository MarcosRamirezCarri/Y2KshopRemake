...
interface LabelTypes {
    ...
    handleSizeChange: (colorIndex: number, sizeIndex: number ,e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => void;


}

const LabelColors: React.FC<LabelTypes> = ({errors, product, handleAddColor, handleRemoveColor, handleRemoveSize, handleAddSize, handleColorChange, handleSizeChange}) =>{
    
 return(
    <div className="flex flex-col w-[100%] max-h-[55vh]">
        ...
        <div className=" overflow-x-hidden overflow-y-auto">
          ...
          {product.colors.map((color, colorIndex) => (
            <div key={colorIndex}>
              ...
              {errors.sizes && (
                <p className="text-pink-950 font-titilium text-sm">{errors.sizes}</p>
              )}
                {errors.duplicateSizes && (
                <p className="text-pink-950 font-titilium self-end text-sm">{errors.duplicateSizes}</p>
              )} {errors.sizesQuantity && (
                <p className="text-pink-950 font-titilium self-end text-sm">{errors.sizesQuantity}</p>
              )}
